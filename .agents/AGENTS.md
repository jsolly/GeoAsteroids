...
- **Local dev (optional):** edit the canonical copy in `.agents/`, run `scripts/refresh-fleet.sh`, commit `fleet/`, then `scripts/refresh-fleet-branch.sh`. In each app repo, run `scripts/update-agents-subtree.sh`.
