...
import { DEBUG, PALETTE, SATELLITE } from '../../src/constants';
import {
  aimAngleToward,
  applyAimJitter,
  clampToRadius,
...
export class SatelliteManager {
  ...
  public createSatellites(count: number, bounds = { radius: 3100 }): SatelliteData[] {
    ...
    for (let i = 0; i < satelliteCount; i++) {
      const satellite = this.spawnSatellite(i, bounds);
      this.satellites.set(satellite.id, satellite);
      created.push(this.toPublic(satellite));
    }

    logger.info(`🛰️ Created ${created.length} satellites`);
    return created;
  }
  ...
  private spawnSatellite(index: number, bounds: { radius: number }): SatelliteInternal {
    const orbitCenter = this.rng.randomPosition({ radius: Math.min(bounds.radius, SATELLITE.BOUNDARY_RADIUS) });
    ...
    return {
      id: `server-sat-${index}`,
      name: SATELLITE_NAMES[index] ?? `Satellite-${index}`,
      position,
      velocity: { x: 0, y: 0 },
      angle: 0,
      exploding: false,
