...
// Test: Laser collision with asteroids
test('laser hits and destroys asteroids', async () => {
  ...
  // Fire lasers at asteroids
  await game.fireLasers(5, 200); // Fire 5 lasers with 200ms delay
  
  // Wait for potential asteroid destruction
  await page.waitForTimeout(1000);
  
  // Take a screenshot for debugging
  const screenshotPath = screenshotManager.getScreenshotPath(
    screenshotManager.getTimestampedFilename('laser-asteroid-collision-test')
  );
  await page.screenshot({ path: screenshotPath });
  
  // Verify lasers were fired (basic check)
  // In a real implementation, you might check for:
  // - Asteroid count reduction
  // - Score increase
