...
test('medium roids do not split', async () => {
  ...
  // Solo finish of a large roid does not produce mediums. Seed a medium by
  // reporting a collision destroy is not available here, so assert the live
  // rule: only biggest asteroids can ever split, and only via two ships.
