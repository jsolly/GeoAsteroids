...
describe('factions for humans and bots', () => {
  ...
  test('opposite-faction lasers still damage humans and bots', () => {
    ...
    const bots = engine.createBots(2) ?? [];
    const emberBot = bots.find((bot) => bot.faction === 'ember') ?? bots[0];
  ...
  test('gameState includes faction for every ship', () => {
    ...
    for (const entity of state.entities) {
      expect(entity.faction === 'ion' || entity.faction === 'ember').toBe(true);
    }
  });
});
