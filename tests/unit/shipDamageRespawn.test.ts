...
// Ship damage and respawn logic is now server-controlled

describe('Ship Damage and Respawn (Server-Controlled)', () => {
  ...
  test('damage and respawn system is now server-controlled', () => {
    // In the new architecture, damage calculation and respawn logic is handled by the server
  ...
  test('ships can handle multiple damage instances', () => {
    const initialHealth = localShip.health;
    localShip.takeDamage(25);
    localShip.takeDamage(25);
    expect(localShip.health).toBeLessThan(initialHealth - 25);
  });

  test('ships can be destroyed', () => {
    localShip.health = 0;
    localShip.exploding = true;
    expect(localShip.exploding).toBe(true);
