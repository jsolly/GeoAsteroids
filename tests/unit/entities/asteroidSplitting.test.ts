...
describe('Collaborative asteroid split', () => {
  ...
  test('two players hit big roid within 1s → split', () => {
    ...
    const first = asteroidManager.registerLaserHit('big-1', 'player-a', ROID.POINTS_LARGE, 0);
    expect(first.outcome).toBe('tagged');
    expect(first.split).toBe(false);
    expect(asteroidManager.getAsteroidCount()).toBe(1);

    const second = asteroidManager.registerLaserHit('big-1', 'player-b', ROID.POINTS_LARGE, 999);
  ...
  test('player and bot hitting a big roid within 1s also splits', () => {
    ...
    asteroidManager.registerLaserHit('big-bot', 'human-1', ROID.POINTS_LARGE, 0);
    const result = asteroidManager.registerLaserHit('big-bot', 'server-bot-1', ROID.POINTS_LARGE, 400);
  ...
  test('solo player destroying a big roid does not split', () => {
    ...
    asteroidManager.registerLaserHit('big-solo', 'player-a', ROID.POINTS_LARGE, 0);
    const result = asteroidManager.registerLaserHit('big-solo', 'player-a', ROID.POINTS_LARGE, 200);
  ...
  test('two players hitting a big roid after the 1s window does not split', () => {
    ...
    asteroidManager.registerLaserHit('big-late', 'player-a', ROID.POINTS_LARGE, 0);
  ...
  test('expired solo tag destroys a big roid without splitting', () => {
    ...
    asteroidManager.registerLaserHit('big-expire', 'player-a', ROID.POINTS_LARGE, 0);
  ...
  test('medium asteroid never splits', () => {
    ...
    const result = asteroidManager.registerLaserHit('medium-1', 'player-a', ROID.POINTS_MEDIUM, 0);
  ...
  test('small asteroid never splits', () => {
    ...
    const result = asteroidManager.registerLaserHit('small-1', 'player-a', ROID.POINTS_SMALL, 0);
  ...
  test('asteroid splitting respects max count limit', () => {
    ...
    asteroidManager.registerLaserHit('big-capped', 'player-a', ROID.POINTS_LARGE, 0);
    const result = asteroidManager.registerLaserHit('big-capped', 'player-b', ROID.POINTS_LARGE, 10);
