...
import { DAMAGE, DEBUG, ROID, SHIP } from '../src/constants';
...
export function isCombatantImmune(state: CombatantState): boolean {
  ...
  if (state.spawnProtectionTimer !== undefined && state.spawnProtectionTimer > 0) {
    ...
    // Client ships omit type and use blinkCount for the visual window.
    return true;
  }
  return false;
}
...
export function isAllowedLaserReporter(reporterId: string, attackerId: string, targetId: string): boolean {
